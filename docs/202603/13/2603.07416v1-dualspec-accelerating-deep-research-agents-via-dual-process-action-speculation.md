...
tldr: 针对深度研究智能体在处理长程任务时推理耗时、延迟高的问题，本文提出了DualSpec框架。该框架基于动作异质性分析，发现搜索（Search）和访问（Visit）动作在推理需求上存在显著差异。DualSpec采用双过程投机策略，结合轻量级置信度语义验证器，在保持模型准确率的同时，实现了显著的端到端推理加速，为高效智能体设计提供了新思路。
selection_source: fresh_fetch
motivation: 现有的投机执行框架因采用统一策略和严格匹配，难以有效降低深度研究智能体在复杂推理和频繁工具调用中的高延迟。
method: 提出DualSpec框架，利用动作异质性进行双过程投机，并引入基于置信度的轻量级语义验证器来优化验证过程。
result: 实验表明，DualSpec在多个基准测试中实现了高达3.28倍的端到端加速，且准确率与全推理智能体相当。
conclusion: 通过针对不同类型的动作实施差异化的投机与验证策略，可以显著提升深度研究智能体的运行效率而不损失性能。
---

## 摘要
基于大语言模型的深度研究智能体在处理长程信息寻求任务方面日益流行，但由于大量的推理和频繁的工具使用，它们往往会产生较高的端到端延迟。投机框架旨在通过将动作执行与推理重叠来降低延迟；然而，现有方法通常依赖于统一的投机策略和严格的动作匹配，这限制了推理加速和鲁棒性。在这项工作中，我们从动作异质性的视角重新审视了深度研究智能体的“投机-验证”范式。我们展示了 Search（搜索）和 Visit（访问）动作在推理和模型能力需求上存在根本差异：基于熵的分析表明，Search 决策具有更高的不确定性，并能从显式推理中显著获益，而 Visit 决策的熵较低，主要取决于模型能力。受此双过程特性的启发，我们提出了 DualSpec，这是一个配备了轻量级、基于置信度的语义验证器的异质投机框架。在多个模型和基准测试上的实验表明，DualSpec 实现了高达 3.28 倍的端到端加速，同时保持了与完全推理智能体相当的准确率。
