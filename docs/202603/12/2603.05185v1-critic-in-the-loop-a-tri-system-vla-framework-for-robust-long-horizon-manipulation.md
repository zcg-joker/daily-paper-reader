...
tldr: 针对视觉机器人操作中高层推理与底层反应式控制难以平衡的问题，本文提出了一种名为“Critic in the Loop”的三系统VLA框架。该框架结合了负责全局推理的VLM、负责快速执行的VLA以及轻量级视觉Critic。Critic通过动态调度，在常规任务中保持VLA的高效执行，并在检测到异常时触发VLM重规划，显著提升了长程任务的鲁棒性和自主性。
selection_source: fresh_fetch
motivation: 现有的VLM推理延迟高，而快速VLA模型在复杂长程任务中缺乏语义深度，难以兼顾实时性与智能性。
method: 提出由VLM大脑、VLA小脑和视觉Critic组成的三系统架构，利用Critic动态监控并根据任务状态在VLM与VLA之间切换控制权。
result: 实验表明，该方法在挑战性的长程操作基准测试中达到了SOTA性能，并有效减少了昂贵的VLM调用。
conclusion: 通过视觉引导的动态调度机制，该框架成功实现了高效、鲁棒且具备自主纠错能力的机器人长程操作。
---
