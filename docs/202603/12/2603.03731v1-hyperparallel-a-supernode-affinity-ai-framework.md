...
motivation: 现有AI框架难以有效利用超节点架构的低延迟互连和统一内存特性，导致编程复杂、负载不均及内存利用率低下。
method: 提出超节点亲和框架HyperParallel，集成自动化分层内存管理、细粒度MPMD并行及声明式并行策略规范等核心技术。
result: 在MindSpore上的实验表明，该框架显著提高了训练和推理效率，同时大幅减少了并行编程与系统调优的开销。
conclusion: 超节点亲和性设计是提升下一代大规模AI模型系统性能和开发效率的必要途径。
---

## 摘要
大规模、稀疏、多模态及智能体 AI 模型的兴起，伴随着硬件向超节点架构的转变，该架构通过超低延迟互连和统一内存池集成了成百上千个加速器。然而，现有的 AI 框架并非为高效利用这些架构而设计，导致了高编程复杂度、负载不均衡以及内存利用率低等问题。在本文中，我们提出了一种超节点亲和的 AI 框架，该框架将超节点视为单一逻辑计算机，并将硬件感知的编排嵌入到框架中。我们的 HyperParallel 架构在 MindSpore 中实现，包含用于自动化分级内存管理的 HyperOffload、用于跨异构工作负载实现细粒度 MPMD 并行的 HyperMPMD，以及用于声明式并行策略规范的 HyperShard。这些技术共同显著提升了训练和推理效率，同时降低了并行编程和系统调优开销，证明了超节点亲和性对于下一代 AI 框架的必要性。
